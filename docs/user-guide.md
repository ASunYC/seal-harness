...
恢复、流式回答、工具调用卡片、任务中止和浏览器审批。侧栏底部 Settings 打开统一设置
中心：General 管理工作区、主题和安全说明；Models 管理 Provider、模型、推理等级与
进程内 API key；Plugins 显示隔离 Profile 的安装清单、兼容状态和缺失适配器，并支持
安装、启停与删除。插件写操作只允许回环 Web Host。API key 只保存在当前进程内存，
服务停止即清除。
