...
describe("loadProfile", () => {
  it("discovers and loads a native ESM profile", async () => {
    ...
    await writeFile(
      join(cwd, "piharness.config.mjs"),
      `export default [{ plugin: { name: "example", setup() {} }, config: { value: 1 } }];\n`,
    );

    const loaded = await loadProfile({ cwd });
    expect(loaded.configPath).toBe(await realpath(join(cwd, "piharness.config.mjs")));
  ...
  it("rejects a malformed profile", async () => {
    const cwd = await temporaryDirectory();
    await writeFile(join(cwd, "piharness.config.mjs"), "export default { plugins: [] };\n");
...
async function temporaryDirectory(): Promise<string> {
  const path = await mkdtemp(join(tmpdir(), "piharness-host-"));
